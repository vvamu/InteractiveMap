...
const ListUser = ({ data, setEditUser }) => {
    const [curUser, setCurUser] = useState(null);
    const handleOpen = () => document.location = ApplicationUrl.User.app.get + data.id;
    const handleEdit = (id) => { document.location = ApplicationUrl.User.app.edit + data.id;}            
    ...
    const handleDelete = (id, name) => {
        ...
        setMessageActionConfirmation(`Вы уверины что хотите удалить "${name}"?`);
    }
        
    const handleConfirm = async () => {
        var res = await userService.deleteAsync(data.id, false);
        setEditUser()
        hadnleCancel();
    ...
    return (
        <li className={classes.item}>
            ...
            <div className={classes.tools}>
                <ButtonIcon imgStyle={{ filter: "invert(1)" }} src={openIcon} alt={"Открыть"} title={"Открыть"} onClick={() => handleOpen(data.id)} />
                {
                    curUser?.roles != "Administrator" ? null : <ButtonIcon imgStyle={{ filter: "invert(1)" }} src={editIcon} alt={"Редактировать"} title={"Редактировать"} onClick={() => handleEdit(data.id)} /> 
                }
                {
                    curUser?.roles != "Administrator" ? null : <ButtonIcon imgStyle={{ filter: "invert(1)" }} src={deleteIcon} alt={"Удалить"} title={"Удалить"} onClick={() => handleDelete(data.id, data.username)} />
                }
